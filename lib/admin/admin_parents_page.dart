                    ...
                    return Container(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                          ...
                          child: const Icon(Icons.family_restroom, color: primaryGreen),
                        ),
                        ...
                        onTap: () => _openParentDialog(
                          ...
                          fullName: fullName,
                          status: status,
                          childrenIds: List<String>.from(data['children'] ?? []),
                        ),
                      ),
                    );
                  },
  ...
  Widget _buildDetailRow(String label, String value) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text("$label: ", style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF5F6771), fontSize: 14)),
          Expanded(child: Text(value, style: const TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF2E3B4E), fontSize: 14))),
        ],
      ),
    );
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(fullName, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800)),
                  ),
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildDetailRow("Status", status == 'disabled' ? 'Dezactivat' : 'Activ'),
                          
                          if (childrenIds.isNotEmpty) ...[
                            const SizedBox(height: 16),
                            const Divider(color: Color(0xFFE6EBEE)),
                            const SizedBox(height: 8),
                            const Text('Copii:', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF5F6771), fontSize: 14)),
                            const SizedBox(height: 12),
                            FutureBuilder<List<DocumentSnapshot>>(
                              future: Future.wait(childrenIds.map((id) => FirebaseFirestore.instance.collection('users').doc(id).get())),
                              builder: (context, csnap) {
                                if (csnap.hasError) return const SizedBox.shrink();
                                if (!csnap.hasData) return const Center(child: CircularProgressIndicator(strokeWidth: 2));
                                
                                final docs = csnap.data!;
                                if (docs.isEmpty) return const Text('Niciun copil înregistrat.');
                                
                                return Column(
                                  children: docs.map((ds) {
                                    final md = ds.data() as Map<String, dynamic>? ?? {};
                                    final cname = (md['fullName'] ?? md['username'] ?? ds.id).toString();
                                    ...
                                    return Container(
                                      ...
                                      child: Row(
                                        children: [
                                          const Icon(Icons.person, size: 20, color: Color(0xFF7AAF5B)),
                                          const SizedBox(width: 10),
                                          Expanded(
                                            child: Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text(cname, style: const TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF2E3B4E))),
                                                Text('user: $cun', style: const TextStyle(color: Colors.black54, fontSize: 12)),
                                              ],
                                            ),
                                          ),
                                        ],
                            ...
                            Expanded(
                              child: ElevatedButton(
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: status == 'disabled' ? const Color(0xFF4CAF50) : const Color.fromARGB(154, 109, 103, 100),
                                ...
                                onPressed: busy ? null : () async {
                                  final ok = await showDialog<bool>(
                                    ...
                                    builder: (_) => AlertDialog(
                                      ...
                                      content: Text("Ștergi definitiv părintele: $username?"),
                                      actions: [
